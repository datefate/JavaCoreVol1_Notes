...
禁止使用 Lombok的 @EqualsAndHashCode, 具有继承关系的子类生成的equals不包含父类的字段

使用Idea自动生成后，如果有继承关系，注意是否需要是否要允许子类参与equals检查，避免子类和和父类的两个实列的       父类字段  拥有完全相同的值时,  父类实列.equals(子类实列) =true。



验证包可以使用 equalsverifier。
